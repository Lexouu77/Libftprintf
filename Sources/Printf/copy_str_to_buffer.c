...
/*   Updated: 2017/11/28 18:03:16 by ahamouda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		*copy_str_to_buffer(char *s, size_t len)
{
	...
	i = 0;
	if (!g_ftprintf_buffer)
		malloc_buffer(len);
	if (g_ftprintf_len + len < g_ftprintf_m_len)
		resize_buffer(len);
	while (i < len)
	{
		g_ftprintf_buffer[g_ftprintf_len + i] = s[i];
		i++;
	}
	g_ftprintf_len += i;
	return (s + i);
...
void		copy_char_to_buffer(char c)
{
	if (!g_ftprintf_buffer)
		malloc_buffer(1);
	if (g_ftprintf_len + 1 < g_ftprintf_m_len)
		resize_buffer(1);
	g_ftprintf_buffer[g_ftprintf_len++] = c;
}
